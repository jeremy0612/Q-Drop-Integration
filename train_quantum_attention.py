"""
QGAT grid search: algorithms × datasets × qubits × Q-Drop hyperparams.
...
Results → training_results/qgat_gridsearch_<timestamp>/
  run_<key>/metrics.json
  results.csv
  summary.json

Usage examples:
  python train_quantum_attention.py
  python train_quantum_attention.py --qubit-options 4 8 --q-depths 1 3
  python train_quantum_attention.py --prune-ratios 0.5 0.8 0.9 --drop-probs 0.3 0.5 0.7
  python train_quantum_attention.py --epochs 50 --folds 5
"""
...
_QGAT_PARAM_PATTERNS = ("vqc.q_weights", "quantum_attention.weights")
...
def _qdrop_combos(algo: str, cfg: GridConfig) -> List[Dict]:
    """Return list of Q-Drop param dicts for a given algorithm."""
    if algo == "baseline":
...
def train_fold(
    ...
    qdrop_cfg = TorchQDropConfig(
        ...
        quantum_param_patterns=_QGAT_PARAM_PATTERNS,
    )
...
def parse_args() -> GridConfig:
    p = argparse.ArgumentParser(description="QGAT grid search — algorithms × qubits × Q-Drop params")
...
def main():
    ...
    out_dir = Path(cfg.output_dir) / f"qgat_gridsearch_{ts}"
    ...
    print("=" * 72)
    print("QGAT GRID SEARCH  —  algorithms × qubits × Q-Drop params")
    print("=" * 72)
    print(f"Datasets        : {list(cfg.datasets)}")
    print(f"Layer depths    : {list(cfg.q_depths)}")
    print(f"Qubit options   : {list(cfg.qubit_options)}")
    ...
    for idx, cell in enumerate(grid, 1):
        ...
        # incremental CSV flush so results are available mid-run
    ...
    with open(summary_path, "w") as f:
        json.dump({"timestamp": ts, "config": asdict(cfg), "results": all_rows}, f, indent=2)

    # ── leaderboard ───────────────────────────────────────────────────────────
    print("\n" + "=" * 80)
    print("LEADERBOARD  (sorted by mean accuracy)")
    print("=" * 80)
    sorted_rows = sorted(all_rows, key=lambda r: r.get("mean_accuracy", 0), reverse=True)
    hdr = f"{'run_key':<52} {'acc':>8} {'±':>6} {'f1':>8} {'roc':>8}"
    print(hdr)
    print("-" * 80)
    for r in sorted_rows:
        ...
        print(
            ...
            f"{r['std_accuracy']:>6.4f} "
            f"{r['mean_f1']:>8.4f} "
            f"{r['mean_roc_auc']:>8.4f}"
        )
